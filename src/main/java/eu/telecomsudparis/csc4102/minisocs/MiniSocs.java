...
public class MiniSocs {
  ...
  public boolean invariant() {
    return nomDuSysteme != null && !nomDuSysteme.isBlank() 
           && utilisateurs != null && reseauxSociaux != null;
  }
   ...
   */
  public void ajouterUtilisateur(final String pseudo,
      final String nom, final String prenom, final String courriel)
  ...
  public void creerReseauSocial(final String pseudoUtilisateur,
    ...
    if (u == null) {
      throw new OperationImpossible("utilisateur inexistant avec ce pseudo " + " (" 
      + pseudoUtilisateur + ")");
    }
    if (u.getEtatCompte().equals(EtatCompte.BLOQUE)) {
      throw new OperationImpossible("le compte est bloqué");
   ...
   * @param pseudoUtilisateur    le pseudo de l'utilisateur à ajouter dans le
   *                             réseau.
   * @param pseudoMembre         le pseudo de l'utilisateur dans ce réseau
   * @param nomReseau            le nom de ce réseau
   * @throws OperationImpossible en cas de problèmes sur les pré-conditions.
  ...
  public void ajouterMembre(final String pseudoUtilisateurMod,
       final String pseudoModerateur, final String nomReseau,
       final String pseudoUtilisateur, final String pseudoMembre, final String etatStrategie)
       throws OperationImpossible {
    ...
    if (etatStrategie != "immédiat" && etatStrategie != "quotidien" 
         && etatStrategie != "pas de notifications") {
      throw new OperationImpossible(
      "Le type ed stratégie doit etre soit 'immédiat' ou 'quotidien' ou 'pas de notifications'");
    }
    ReseauSocial r = reseauxSociaux.get(nomReseau);
    if (r == null) {
    ...
    if (mod == null) {
      throw new OperationImpossible("l'utilisateur doit appartenir à ce réseau social(" 
       + nomReseau + ")");
    }
    Utilisateur modU = utilisateurs.get(pseudoUtilisateurMod);
    if (modU == null) {
      throw new OperationImpossible("l'utilisateur doit avoir une compte sur le réseau");
    }
    ...
    if (!mod.estModerateur()) {
      throw new OperationImpossible(
      "l'utilisateur doit etre un modérateur de ce réseau social et non pas seulement un membre");
    }
    if (modU.getMembres().get(pseudoModerateur) != mod) {
      throw new OperationImpossible("l'utilis ne correspond pas au compte moderateur mentionné");
    }
    Utilisateur u = utilisateurs.get(pseudoUtilisateur);
    ...
    // verifier si l'utilisateur existe dans le réseau social sous un autre
    // pseudonyme
    for (Map.Entry<String, Membre> entry : u.getMembres().entrySet()) {
      Membre it = entry.getValue();
      if (r.equals(it.getReseauSocial())) {
        throw new OperationImpossible("L'utilis existe dans le réseau social sous un autre pseudo");
      }
    }
  ...
  /**
   * poster un message dans un reseau social.
   *
   */
  
  public Long posterMessage(final String pseudoUtilisateur,
            final String contenu, final String pseudoMembre,
            final String nomReseau) throws OperationImpossible, InterruptedException {
    ...
    if (m == null) {
      throw new OperationImpossible("Membre avec ce pseudonyme (" + pseudoMembre
              + " ) n'existe pas dans le reseau (" + nomReseau + "). ");
    }
    if (!r.estOuvert()) {
    ...
    if (u.getMembres().get(pseudoMembre) != m) {
      throw new OperationImpossible("le pseudo du membre indiqué ne correspond pas à cette utilis");
    }
  ...
  /**
   * Un modérateur accepte un Message (en ce moment posté et en attente), si
   * acceptation est true alors l'etatMessage devient VISIBLE si non l'étaMessage
   * devient REJETÉ .
   *
   * @param pseudoModUtilisateur le pseudonyme de l'utilisateur qui va accepter
   *                              le message.
   ...
   * @throws OperationImpossible en cas de problème sur les pré-conditions.
   */
  public void modereMessage(final String pseudoModUtilisateur, final String pseudonymeModerateur,
      final String nomReseau, final Long idMessage, final boolean acceptation)
    ...
    if (m == null) {
      throw new OperationImpossible("Membre avec ce pseudonyme (" + pseudonymeModerateur
         + " ) n'existe pas dans le reseau (" + nomReseau + "). ");
    }
    ...
    if (!m.estModerateur()) {
      throw new OperationImpossible("vous n'êtes pas modérateur,"
              + " vous ne pouvez pas moderer un message");
    }
    Message mess = rs.getMessages().get(idMessage);
    if (mess == null) {
      throw new OperationImpossible("Message avec cet id (" 
      + idMessage + " ) n'existe pas dans le reseau. ");
    }
    ...
    if (acceptation) {
      rs.getProducteurMessagePoste().submit(new Notification("Nouveau message posté! "
                 + "Ce message est posté le :"
                   + LocalDateTime.now() + " -- " + mess.getContenu() + "--"));
    }
    Thread.sleep(100);
    assert invariant();
  }
   ...
   *
   * @param pseudoUtilisateur le pseudonyme de l'utilisateur qui va accepter le
   *                          message.
  ...
  public void fermerReseau(final String pseudoUtilisateur, 
        final String pseudoMembre, final String nomReseau)
        throws OperationImpossible {
    ...
    if (m == null) {
      throw new OperationImpossible(
                "Membre avec ce pseudonyme (" + pseudoMembre + " ) n'existe pas dans ce réseau. ");
    }
    if (!m.estModerateur()) {
      throw new OperationImpossible("vous n'êtes pas modérateur,"
                         + "vous ne pouvez pas fermer le réseau");
    }
    Utilisateur u = utilisateurs.get(pseudoUtilisateur);
    if (u == null) {
      throw new OperationImpossible("utilisateur inexistant");
